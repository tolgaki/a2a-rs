...
use a2a_core::{JsonRpcResponse, Message, Part, Role, TaskState, TextPart};
...
async fn test_message_send() {
    ...
    let task: a2a_core::Task = serde_json::from_value(result).unwrap();
...
async fn test_message_send_and_poll() {
    ...
    let rpc_response = response_json(response).await;
    let task: a2a_core::Task = serde_json::from_value(rpc_response.result.unwrap()).unwrap();
...
async fn test_agent_card_endpoint() {
    ...
    assert!(card.endpoint.contains("/v1/rpc"));
}
...
async fn test_task_cancel() {
    ...
    // First create a task
    let message = Message {
        id: "msg-cancel".to_string(),
        role: Role::User,
        parts: vec![Part::Text(TextPart {
            text: "To be cancelled".to_string(),
        })],
        context_id: None,
        reference_task_ids: None,
        metadata: None,
...
async fn test_echo_response_content() {
    ...
    if let Part::Text(text_part) = &agent_msg.parts[0] {
        assert!(text_part.text.contains("echo:"));
        assert!(text_part.text.contains("Hello World"));
    } else {
